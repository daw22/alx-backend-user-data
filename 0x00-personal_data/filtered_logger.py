...
def get_logger() -> logging.Logger:
    """
    Returns a configured logger named "user_data" with a
    StreamHandler and custom Formatter.
...
def main():
    """
    Get a database connection using get_db
    retrieve all rows in the users table and display
    each row with filtered format
    """
    db = get_db()
    cursor = db.cursor()
